...
def near(img):
    ...
    for i in range(len(contours3)):
        ...
        l = np.sqrt(np.power(x_center-351,2)+np.power(y_center-107,2))
        lenght.append(l)
...
def callback0(imgmsg):
    ...
    cv.circle(img, (351,107), 5, (255, 0, 0))
    # right_hand = Float32MultiArray(data = [x_center-351,y_center-107,theta] )
